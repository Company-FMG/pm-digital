...
export default function InfoViatura({
    ...
    const exitViatura = async () => {
        try {
            const response = await axios.put(`${api_url}/policiais/out/${idViatura}`, {}, {
                headers: {
            ...
            if (response.status === 200) {
                console.log('Saiu da viatura com sucesso!');
                localStorage.removeItem("placaViatura")
    ...
    return (
        <>
            <div onClick={handleMoreClick} role="button" className="flex flex-col bg-bluemike w-full p-6 rounded-md justify-between">
                ...
                <div
                    ...
                    <div className="flex flex-col w-full">
                        ...
                        <button onClick={exitViatura} className="w-full text-bluemike font-poppins bg-white p-4 rounded-lg font-bold">
                            Sair da viatura
                        </button>
                    </div>
                    </div>
